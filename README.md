...
| 方式         | 触发条件                     | 适用                                                             |
| ------------ | ---------------------------- | ---------------------------------------------------------------- |
| **空闲分帧** | 静默超过 N 毫秒（默认 10ms） | 没有帧尾标记的二进制协议。Modbus RTU 的 3.5 字符间隔就是这个思路 |
| **原样分块** | 把空闲分帧设为 `0`           | 排查「数据到底是怎么到达的」。零延迟、行为完全可预测             |
...
- **换行分帧只在 TXT 视图下生效**。切到 HEX 时自动回落到空闲/原样，偏好本身保留，切回来还在。
...
- **接收分帧**：空闲超时（默认 10ms，设为 0 即原样分块）、按换行符分帧（仅 TXT 视图，与空闲分帧互斥）——详见下文
...
自动化测试覆盖 core 层与组件层，共 333 个用例（core 层语句覆盖率 98%）：
